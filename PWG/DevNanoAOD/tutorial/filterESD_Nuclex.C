...
void filterESD_Nuclex()
{
  ...
  AliAnalysisTaskNanoAODFilterSelection* myFilterTask = AliAnalysisTaskNanoAODFilterSelection::AddTask();
  
  ///TODO: put these cuts in a separate AliAnalysisCut class, the AliAnalysisTaskNanoAODFilterSelection should just apply some generic track cuts (maybe a list)
  double nucleiTPCpt[4][2]{{0.6,1.4},{1.,1.8},{1.,10.},{1.,10.}};
  double nucleiTOFpt[4][2]{{0.6,10.},{1.,10.},{1.,10.},{1.,10.}};
  double nucleiTOFsigma[4]{10.,10.,-1.,-1.};
  AliESDtrackCuts* nucleiCuts[4]{AliESDtrackCuts::GetStandardITSTPCTrackCuts2011(false),AliESDtrackCuts::GetStandardITSTPCTrackCuts2011(false), new AliESDtrackCuts,AliESDtrackCuts::GetStandardITSTPCTrackCuts2011(false)};
  nucleiCuts[2]->SetMinNClustersTPC(50);
  nucleiCuts[2]->SetMaxChi2PerClusterTPC(4);
  nucleiCuts[2]->SetAcceptKinkDaughters(kFALSE);
  nucleiCuts[2]->SetRequireTPCRefit(true);

  for (int iN{0}; iN < 4; ++iN) {
    myFilterTask->TriggerOnSpecies(AliPID::EParticleType(AliPID::kDeuteron+iN), nucleiCuts[iN], 5., nucleiTPCpt[iN], nucleiTOFsigma[iN], nucleiTOFpt[iN]);
  }
  // OCDB
  ...
  // v0Finder->SetTrackTriggerCuts(nucleiCuts[2], AliPID::kHe3, 5.); ///TODO: fix this, is buggy.
  ...
  nanoFilterTask->SetVarListHeader("Centr,OfflineTrigger,MagField");
  // track level
